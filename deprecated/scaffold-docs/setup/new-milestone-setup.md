...
1. **Milestone-specific planning (required)**
   ...
   - Add one markdown file per topic as needed (e.g. auth.md, project-rules.md, tech-stack.md). Use the files in `_docs/planning/milestones/00-initial-milestones/` (README.md and topic files such as auth.md, project-rules.md in the same folder—flat layout) only as a **guide** for content types—the breakdown varies by milestone (e.g. skip auth for a dev-experience milestone; add 3D/model docs for a WebGL milestone). The README links to these files.
   - **Pause for review.**
